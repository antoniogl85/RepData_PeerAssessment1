...
for (i in 1:nrow(data)){
    if (is.na(data[i,"steps"])){
        data2[i, "steps"] <- interval.avg[interval.avg$interval == data[i,"interval"], "steps"]
    }
}
```
...
data2$type.date <- as.factor(ifelse(weekdays(as.Date(data2$date), abbreviate = T) %in% c("Sat", "Sun"), "weekend", "weekday"))
